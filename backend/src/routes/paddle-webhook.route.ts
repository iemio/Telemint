        ...
        if (signature && rawRequestBody) {
            ...
            if (eventData.eventType) {
                if (eventData.eventType === "subscription.activated") {
                    const billingStart =
                        eventInfo?.current_billing_period?.starts_at;
                    const billingEnd =
                        eventInfo?.current_billing_period?.ends_at;
                    ...
                    const { error } = await resend.emails.send({
                        from: "Telemint <onboarding@resend.dev>", // change when you have your own domain@garib
